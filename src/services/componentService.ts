...
export class ComponentService {
    ...
    async registerAll() {
        ...
        await vscode.workspace.findFiles("**/src/**/*.{vue}").then(files => {
            ...
            files.forEach(async file => {
                ...
                await vscode.workspace.fs.readFile(file).then(async array => {
                    ...
                    if (document.firstElementChild?.children?.length! > 1) {
                        const wrapper = document.createElement('div');
                        document.firstElementChild?.appendChild(wrapper);

                        let desc = document.firstElementChild?.children;
                        ...
                        if (name === "App.vue") {
                            wrapper?.setAttribute("odin-id", this.generateID());
                        }
                        else {
                            wrapper?.setAttribute("component-id", this.generateID());
                    ...
                    for (let i = 0; i < children.length; i++) {
                        ...
                        if (i === 1) {  //if root of component file
                            ...
                            if ((!valid && elt?.tagName !== "template")) {  //if root is component
                                ...
                                if (name === "App.vue") {
                                    wrapper?.setAttribute("odin-id", id);
                                }
                                else {
                    ...
                    await vscode.workspace.fs.writeFile(file, te.encode(source));

                });
                await this.createConfigFile();
                await this.createComponentMap(componentMap);
    ...
    async createComponentMap(componentMap: Map<string, { uri: vscode.Uri; name: string; }>) {
        ...
        await vscode.workspace.fs.writeFile(configUri, te.encode(json));
